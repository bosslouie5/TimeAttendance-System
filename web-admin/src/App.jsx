...
function App() {
  ...
  const loadInitialData = async () => {
    try {
      const [e, b, o, l, pt, sc, asg] = await Promise.all([
  ...
  return (
    <div className="app-container">
                      ...
                      <td style={{textAlign:'center'}}>
                        <button
                          onClick={() => {
                            ...
                            const existing = assignments.find(a => a.employeeId === e.employeeId);
      ...
      {/* ASSIGN BRANCH MODAL (MULTI-BRANCH) */}
      {isAssignModalOpen && selectedAssignEmp && (
        <div className="modal-overlay">
          <div className="modal-content fade-in" style={{maxWidth:'550px'}}>
            ...
            <div className="form-group">
              <label style={{marginBottom: '15px', display: 'block'}}>SELECT BRANCHES TO AUTHORIZE</label>
              <div style={{background: '#0f172a', borderRadius: '15px', border: '1px solid #334155', maxHeight: '300px', overflowY: 'auto', padding: '10px'}}>
                {departments.length === 0 ? (
                  <div style={{padding: '20px', textAlign: 'center', color: '#64748b'}}>Walang nakarehistrong branch.</div>
                ) : (
                  departments.map(d => {
                    ...
                    return (
                      ...
                      >
                        ...
                        />
                        <div>
                          <div style={{fontWeight: '900', color: isChecked ? '#60a5fa' : 'white', fontSize: '0.9rem'}}>{d.name}</div>
                          <div style={{fontSize: '0.7rem', color: '#64748b'}}>Radius: {d.radiusMeters}m</div>
                        </div>
                      </div>
                    );
                  })
            ...
            <div style={{display:'flex', gap:'15px', marginTop:'30px'}}>
               <button onClick={saveAssignment} className="btn-blue" style={{flex:1, padding: '18px', fontSize: '1rem', fontWeight: '900'}}>COMMIT ASSIGNMENTS</button>
