...
function normalizeIpOrHost(value) {
  if (!value || typeof value !== 'string') return '';
  let normalized = value.trim().toLowerCase();

  // Handle URLs like http://1.2.3.4:4002 or https://example.com
  ...
  // Strip port suffix from raw host/IP values
...
function matchIp(client, allowed) {
  ...
  if (allowed === '*' || allowed.toUpperCase() === 'ANY') return true;

  const normalizedClient = normalizeIpOrHost(client);
  const allowedList = allowed
    .split(',')
    .map(i => normalizeIpOrHost(i.trim()))
    .filter(Boolean);

  return allowedList.some(a => {
    if (a.includes('*')) {
      const regex = new RegExp('^' + a.replace(/\./g, '\\.').replace(/\*/g, '.*') + '$', 'i');
      return regex.test(normalizedClient);
    }
    return a === normalizedClient;
  });
}
