...
node -e "const fs=require('fs'); const path=require('path'); const v='!NEW_V!'; const meta={version:v, downloadUrl:'/api/master/download-apk/TimeKey_Master.apk', releaseDate:new Date().toISOString(), notes:'System Update v'+v}; ['backend/apks','apks'].forEach(d=>{ if(!fs.existsSync(d)) fs.mkdirSync(d,{recursive:true}); fs.writeFileSync(path.join(d,'latest-version.json'), JSON.stringify(meta, null, 2)); });"
